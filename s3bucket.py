...
class S3Bucket:
    ...
    def upload_file(self, file: str, metadata: dict = None):
        """Upload file to S3 bucket

        Args:
            file (str): Full path of file to upload
            metadata (dict): S3 metadata to add to file
        """
        key = file.lstrip("/")
        extra_args = {}
    ...
    def upload_fileobj(self, fileobj, filename: str, metadata: dict = None):
        """Upload file to S3 bucket

        Args:
            fileobj (object): File object to upload
            filename (str): S3 key to use for file
            metadata (dict): S3 metadata to add to file
        """
        key = filename.lstrip("/")
        extra_args = {}
        ...
        try:
            ...
            self.s3_client.upload_fileobj(fileobj,
                                          Bucket=self.bucket_name,
                                          Key=key,
                                          ExtraArgs=extra_args)
            logger.info(
